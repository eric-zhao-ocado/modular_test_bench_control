...
def automated_routine():
    ...
    x_pos = tk.Scale(jog_arm_window, from_=0, to=500+90, orient=tk.HORIZONTAL, resolution=0.001, length=500)
    x_pos.pack()
    x_pos.set((arm_mover[1] + arm_mover[0])/ (2.0 ** 0.5) + tb_const.X_OFFSET)
    
    y_pos = tk.Scale(jog_arm_window, from_=0, to=580+470, orient=tk.HORIZONTAL, resolution=0.001, length=500)
    y_pos.pack()
    y_pos.set((arm_mover[1] - arm_mover[0]) / (2.0 ** 0.5) + tb_const.Y_OFFSET)
    
    z_pos = tk.Scale(jog_arm_window, from_=-180+height, to=500, orient=tk.HORIZONTAL, resolution=0.001, length=500)
    z_pos.pack()
    ...
    while not next_window_event.is_set():
        ...
        slider_y = y_pos.get()
        slider_z = z_pos.get()
        arm_mover[0] = rtn_mtrx[0][0] * (slider_x-tb_const.X_OFFSET) + rtn_mtrx[0][1] * (slider_y-tb_const.Y_OFFSET)
        arm_mover[1] = rtn_mtrx[1][0] * (slider_x-tb_const.X_OFFSET) + rtn_mtrx[1][1] * (slider_y-tb_const.Y_OFFSET)
