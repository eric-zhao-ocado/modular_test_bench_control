...
class Robot(ABC):
    ...
    def send_cmd(self, cmd):
        ...
        # 1. send
        # print(f'{"Sending data:":<31}{time.perf_counter()}')
        self.comm_sock.sendall(cmd.encode())
        # print(f'{"Finished sending data:":<31}{time.perf_counter()}')
        # 2. wait for a result
        resp = self.comm_sock.recv(self.sock_buff_sz).decode()
        # print(f'{"Response received:":<31}{time.perf_counter()}')
    ...
    def move(
        ...
        # call send_cmd
        # print(f'{"Move function sending command:":<31}{time.perf_counter()}')
        self.send_cmd(cmd)
